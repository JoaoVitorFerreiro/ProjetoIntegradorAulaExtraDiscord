...
async function fazerLogin() {
    ...
    try {
        ...
        document.getElementById('user-name').textContent = `Bem-vindo, ${clienteLogado.nome}!`;
...
async function carregarServicos() {
    try {
        ...
        servicosDiv.innerHTML = servicos.map(servico => `
...
async function carregarBarbeiros() {
    try {
        ...
        barbeirosDiv.innerHTML = barbeiros.map(barbeiro => `
            <div class="option-item" onclick="selecionarBarbeiro(${barbeiro.id})">
...
function selecionarServico(servicoId) {
    ...
    if (barbeiroSelecionado && document.getElementById('data-agendamento').value) {
...
function updateConfirmButton() {
    ...
    btnConfirmar.disabled = !(servicoSelecionado !== null && barbeiroSelecionado !== null && horarioSelecionado);
}
...
async function buscarHorarios() {
    ...
    try {
        const response = await apiRequest(`/horarios-disponiveis?barbeiroId=${barbeiroSelecionado}&data=${data}&servicoId=${servicoSelecionado}`);
        const horariosDisponiveis = response.horariosDisponiveis;

        const horariosDiv = document.getElementById('horarios-list');

        if (horariosDisponiveis.length === 0) {
            horariosDiv.innerHTML = '<p style="text-align: center; padding: 20px; color: #666;">Nenhum horário disponível para esta data</p>';
        } else {
            horariosDiv.innerHTML = `
                <div style="margin-bottom: 15px; padding: 10px; background: #f0f8ff; border-radius: 8px; text-align: center;">
                    <strong>Horários disponíveis para ${new Date(data).toLocaleDateString('pt-BR')}:</strong>
                </div>
        ...
        const horariosDiv = document.getElementById('horarios-list');
        horariosDiv.innerHTML = '<p style="text-align: center; padding: 20px; color: #e74c3c;">Erro ao carregar horários</p>';
    }
...
async function carregarMeusAgendamentos() {
    ...
    try {
        const agendamentos = await apiRequest(`/agendamentos/cliente/${clienteLogado.telefone}`);
        ...
        if (agendamentos.length === 0) {
            ...
            agendamentosDiv.innerHTML = agendamentos.map(agendamento => `
                <div class="agendamento-item">
                    ...
                    <p><strong>Data:</strong> ${new Date(agendamento.dataHora).toLocaleDateString('pt-BR')}</p>
                    <p><strong>Horário:</strong> ${new Date(agendamento.dataHora).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</p>
        ...
        relatorioDiv.innerHTML = `
            ...
            ${agendamentos.map(agendamento => `
                <div class="agendamento-item">
                    ...
                    <p><strong>Barbeiro:</strong> ${agendamento.barbeiro}</p>
                    <p><strong>Data:</strong> ${new Date(agendamento.dataHora).toLocaleDateString('pt-BR')} às ${new Date(agendamento.dataHora).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</p>
                    <p><strong>Preço:</strong> R$ ${agendamento.preco.toFixed(2)}</p>
                </div>
            `).join('')}
        `;
...
async function carregarFaturamento() {
    try {
        ...
        relatorioDiv.innerHTML = `
            ...
            ${Object.entries(faturamento).map(([barbeiro, dados]) => `
                <div class="faturamento-item">
                    <h4>👨‍💼 ${barbeiro}</h4>
                    <p><strong>Total de Atendimentos:</strong> ${dados.totalAtendimentos}</p>
                    <p><strong>Faturamento Total:</strong> R$ ${dados.faturamentoTotal.toFixed(2)}</p>
                    <p><strong>Média por Atendimento:</strong> R$ ${(dados.faturamentoTotal / dados.totalAtendimentos).toFixed(2)}</p>
                </div>
            `).join('')}
        `;
    } catch (error) {
